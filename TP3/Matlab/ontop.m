function ontop(ifigs)
%
% puts Figure ifig always on top; 
% with no arguments put ALL existing Figures always on top
%
if nargin==0
    ifigs = findall(0,'type','figure')';
end
%
...
%
if not(isempty(ifigs))
    for ifig=[ifigs.Number]
        figure(ifig);
        WinOnTop(ifig);
    end
end
